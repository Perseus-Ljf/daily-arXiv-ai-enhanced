...
Method: 提出DySCo框架：1) 熵引导动态采样机制自主识别并保留高熵片段，压缩冗余趋势；2) 分层频率增强分解策略分离高频异常和低频模式；3) 跨尺度交互混合器动态融合全局上下文与局部表示。

Result: 实验表明DySCo可作为通用即插即用模块，显著增强主流模型捕捉长时相关性的能力，同时降低计算成本。

Conclusion: DySCo框架通过动态语义压缩有效解决了时间序列预测中长回望窗口带来的噪声和冗余问题，提升了模型的长时依赖捕捉效率。
...
TL;DR: 提出端到端框架训练LLM进行供应链中断概率预测，显著优于GPT-5等基线，实现决策就绪信号


<details>
  <summary>Details</summary>
Motivation: 供应链中断预测面临低频高影响事件难以从噪声数据中可靠推理的挑战，通用模型需要任务特定适应

Method: 引入端到端框架，使用实际中断结果作为监督训练LLM生成校准概率预测，无需显式提示即可诱导结构化概率推理

Result: 模型在准确性、校准度和精确度上显著优于GPT-5等强基线，训练诱导更结构化可靠的概率推理
...
TL;DR: 提出在线机器学习加速的多分辨率优化框架，用于工业过程集成能源系统的设计与验证，通过ML引导的多分辨率控制减少架构到操作性能差距42%，同时降低高保真模型评估需求34%。


<details>
  <summary>Details</summary>
Motivation: 工业过程集成能源系统的可靠设计需要在多个保真度级别上进行优化和验证，但跨保真度的模型不匹配会掩盖性能损失来源，并使架构到操作性能差距的量化变得复杂。

Method: 提出在线机器学习加速的多分辨率优化框架：1) 多目标架构优化选择系统配置和组件容量；2) ML加速的多分辨率滚动时域最优控制策略，基于预测不确定性自适应调度优化分辨率，使用精英低保真解热启动高保真求解。

Result: 在1MW工业热负荷的试点能源系统中，多分辨率策略相比基于规则的控制器将架构到操作性能差距减少42%，相比无ML指导的多保真度方法减少34%的高保真模型评估需求。
...
TL;DR: 论文发现条件流匹配(CFM)的标准训练损失在物理应用中具有误导性，提出了JetPrism框架和多指标评估协议来确保生成模型与真实数据的统计一致性。


<details>
  <summary>Details</summary>
Motivation: 高保真蒙特卡洛模拟和复杂逆问题计算成本高但至关重要。条件流匹配(CFM)能加速这些任务，但其标准训练损失在物理应用中具有误导性，无法可靠指示真实收敛和物理保真度。

Method: 设计了JetPrism框架作为可配置的CFM生成替代模型，用于评估无条件生成和条件探测器展开。使用合成压力测试和Jefferson Lab运动学数据集，提出多指标评估协议，包括边际和成对χ²统计、W₁距离、相关矩阵距离(D_corr)和最近邻距离比率(R_NN)。

Result: 研究表明物理感知指标在标准损失收敛后仍持续显著改善，标准损失过早达到平台期。多指标评估协议确保生成模型与真实数据精确统计一致而不记忆训练集。

Conclusion: 领域特定评估必须取代通用损失指标。JetPrism作为可靠的生成替代模型，可扩展到核物理以外的参数生成和复杂逆问题，在医学成像、天体物理、半导体发现和量化金融等领域有应用潜力。
...
TL;DR: 提出ReDiRect框架，通过模糊分区将大规模交易图分解为可管理组件，在无监督设置下检测洗钱活动，显著提升效率和减少误报


<details>
  <summary>Details</summary>
Motivation: 现有洗钱检测方法存在局限性：洗钱者通过模仿正常交易模式逃避检测；现有算法难以处理大规模复杂数据；基于风险规则的刚性系统产生过多误报信号

Method: 提出ReDiRect框架（REduce, DIstribute, and RECTify）：1）在无监督设置下将大规模交易图模糊分区为更小可管理组件；2）分布式处理实现快速计算；3）定义改进的评估指标以更好捕捉洗钱模式有效性

Result: 在真实Libra数据集和IBM Watson合成数据集上验证，框架性能优于现有和最先进技术，特别是在效率和实际应用性方面表现优异

Conclusion: ReDiRect框架通过新颖的无监督问题表述和分布式处理方法，有效解决了洗钱检测中的规模、复杂性和误报问题，具有实际应用价值
...
TL;DR: 本教程介绍贝叶斯优化(BO)作为科学发现中假设-实验-优化循环的自动化概率框架，通过替代模型和采集函数指导实验设计，平衡探索与利用，加速材料科学、催化、有机合成等领域的科学发现。


<details>
  <summary>Details</summary>
Motivation: 传统科学发现依赖直觉的、临时的假设-实验-优化循环，这种方法资源浪费严重、设计效率低下且容易错过关键洞见。需要一种原则性的概率驱动框架来自动化和形式化这一核心科学循环。

Method: 贝叶斯优化框架：1) 将科学发现形式化为优化问题；2) 使用替代模型（如高斯过程）将经验观察建模为演化假设；3) 使用采集函数指导实验选择，平衡已知知识的利用和未知领域的探索；4) 包含批处理实验、异方差性、上下文优化和人机交互等关键技术扩展。

Result: 通过催化、材料科学、有机合成和分子发现等领域的案例研究展示了BO在现实世界中的有效性。教程提供了分层内容，使跨学科研究人员能够设计更高效的实验，加速原则性科学发现。

Conclusion: 贝叶斯优化为科学发现提供了一个原则性的概率驱动框架，能够自动化假设-实验-优化循环，消除猜测和手动试错，显著提高实验效率，加速跨学科科学发现进程。
...
Method: 重新审视干扰最小化框架，发现在特定条件下，协方差矩阵可以直接从差异矩阵估计，无需数据。该方法在视觉和语言基准测试中验证，模型参数范围从86M到7B

Result: 在视觉和语言基准测试中，该方法优于先前无需数据的最先进合并方法，验证了从差异矩阵估计协方差矩阵的有效性

Conclusion: 提出了一种理论严谨且无需数据的模型合并方法，通过从差异矩阵直接估计协方差矩阵，既保持了干扰最小化框架的理论优势，又获得了启发式方法的实践便利性
...
TL;DR: 提出SECURE框架，通过定义和执行模型鲁棒性来增强事故预测系统的稳定性，在对抗扰动下保持预测和特征空间的一致性


<details>
  <summary>Details</summary>
Motivation: 现有事故预测模型（如CRASH）在面临微小输入扰动时表现出显著的不稳定性，预测和潜在表示存在波动，这对安全关键系统的可靠性构成严重风险
...
Conclusion: SECURE框架通过形式化定义和执行模型鲁棒性，有效解决了事故预测系统在现实扰动下的稳定性问题，为安全关键应用提供了更可靠的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 传统多元霍克斯过程的最大似然估计计算复杂度为O(N²)，线性指数霍克斯过程虽有O(N)递推关系，但现有方法采用顺序计算，无法充分利用现代GPU的并行计算能力。

Method: 将霍克斯过程强度表示为稀疏转移矩阵的乘积，这些矩阵支持线性时间关联乘法，从而可通过并行前缀扫描进行计算。该方法实现了完全并行的最大似然估计算法，支持批处理以保持恒定内存使用。

Result: 在模拟和真实数据集上实现了数量级的加速，可扩展到数千个节点和数千万个事件，远超先前工作的规模。提供了实现这些优化的开源PyTorch库。

Conclusion: 该方法为线性指数霍克斯过程提供了一种简单且可大规模并行化的精确最大似然估计算法，不引入任何额外假设或近似，保持了模型的简单性和可解释性，显著提升了计算效率。
...
Method: 使用Malliavin微积分将反事实条件化为涉及Malliavin量的无条件期望之比，推导Langevin结构的Malliavin导数及其伴随Skorohod积分公式，提出具体算法实现反事实梯度估计

Result: 通过Malliavin微积分方法克服了反事实梯度估计的困难，恢复了标准估计速率，避免了传统方法的低效问题

Conclusion: 提出的基于Malliavin微积分的被动Langevin算法为自适应逆强化学习提供了一种有效的反事实梯度估计方法，解决了传统方法效率低下的问题
...
TL;DR: 提出基于残差的离线强化学习框架，通过显式纳入转移动态估计误差来缓解分布偏移问题，适用于一般状态和动作空间


<details>
  <summary>Details</summary>
Motivation: 离线强化学习在无需与环境交互的情况下从历史数据中学习策略，对于高风险应用尤为重要。现有方法通常依赖严格的数据覆盖假设且易受分布偏移影响，需要更鲁棒的解决方案。

Method: 提出基于残差的离线RL框架，定义基于残差的贝尔曼最优算子，该算子通过利用经验残差将转移动态估计误差显式纳入策略优化。证明该贝尔曼算子是压缩映射，并确定其固定点具有渐近最优性和有限样本保证的条件。进一步开发基于残差的离线深度Q学习（DQN）算法。

Result: 在随机CartPole环境中验证了基于残差的离线DQN算法的有效性，展示了该方法在缓解分布偏移问题上的优势。

Conclusion: 基于残差的离线RL框架通过显式处理转移动态估计误差，为一般状态和动作空间提供了更鲁棒的离线强化学习解决方案，缓解了传统方法对数据覆盖的严格依赖和分布偏移问题。
...
TL;DR: Qumphy项目报告介绍了六个与PPG信号相关的医学问题作为基准问题，并描述了适用的基准数据集及其使用方法


<details>
  <summary>Details</summary>
Motivation: 开发量化机器学习算法在医学应用中不确定性的方法，特别是在PPG信号分析和处理方面，需要建立标准化的基准问题和数据集

Method: 确定六个与PPG信号相关的医学问题作为基准问题，收集和描述适用的基准数据集，并规范其使用方法
...
<details>
  <summary>Details</summary>
Motivation: 现代LLM在测试时通过重复采样等方式进行扩展，导致推理成本随模型大小和样本数增长。传统的预训练缩放定律（如Chinchilla）未考虑这种测试时扩展成本，因此需要新的缩放定律来联合优化预训练和测试时决策。

Method: 提出了Train-to-Test（T²）缩放定律，将预训练缩放定律现代化，纳入用于测试时扩展的pass@k建模，然后在固定端到端预算下联合优化模型大小、训练token数和推理样本数。通过任务损失和任务准确度两种建模方法验证鲁棒性。

Result: 在八个下游任务上发现：当考虑推理成本时，最优预训练决策会大幅转向过训练区域，远超标准预训练缩放套件的范围。通过在T²缩放预测的最优区域预训练严重过训练的模型，验证了这些模型相比仅基于预训练缩放的模型有显著更强的性能。研究还表明这些发现在后训练阶段仍然成立。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型的权重内学习在内部化知识的演绎推理方面存在局限（如反转诅咒），而上下文学习则展现出强大的潜在泛化能力。现有基于训练时数据增强的方法存在任务特定性、扩展性差且无法泛化到分布外知识的问题，因此需要探索新的解决方案。

Method: 使用基于正确性反馈的强化学习训练语言模型，使其能够生成长的思维链，利用测试时计算（"思考"）来改善权重内知识的潜在泛化能力。

Result: 该方法不仅解决了分布内知识的许多潜在泛化失败实例，而且与数据增强基线不同，能够泛化到未经强化学习训练的新知识。然而，在纯反转任务中，思考无法直接实现知识反转，但思考模型的生成-验证能力使其表现显著高于随机水平。不过，由于事实自验证的脆弱性，思考模型在该任务上的表现仍远低于上下文学习。

Conclusion: 测试时思考是改善语言模型潜在泛化能力的灵活且有前景的方向，能够弥补权重内学习在演绎推理方面的不足，但仍有改进空间。
...
Method: 1. 使用模型预测路径积分控制（MPPI）实例化MPC；2. 通过拟合价值迭代训练终端Q函数，使学习到的价值函数与规划器对齐，并隐式扩展有效规划时域；3. 引入摊销预热启动策略，从在线观测中回收规划的开环动作序列以计算批量MPPI价值目标；4. 基于场景的规划，使用为下一步预测精度训练的动力学模型集合。

Result: 软MPCritic在经典和复杂控制任务中通过鲁棒的短时域规划实现有效学习，在保持解质量的同时实现计算可行性，为合成MPC策略提供了实用且可扩展的蓝图。

Conclusion: 软MPCritic通过结合RL和MPC的优势，解决了大规模融合的计算挑战，为在策略提取和直接长时域规划可能失败的场景中合成MPC策略提供了实用框架。
...
TL;DR: ES与GRPO在LLM微调中达到相似任务性能，但产生几何结构不同的参数空间解，ES更新更大更分散，GRPO更新更小更局部，两者解线性连接无损失障碍


<details>
  <summary>Details</summary>
Motivation: 研究进化策略(ES)与基于梯度的强化学习(GRPO)在LLM微调中是否产生相似参数空间解，尽管任务性能可比，这对遗忘和知识保留有重要影响

Method: 在四个任务上比较ES和GRPO，包括单任务和顺序持续学习设置，分析更新大小、KL散度漂移、解空间几何结构，并开发ES分析理论解释现象

Result: ES在单任务准确率上匹配或超过GRPO，在顺序学习中预算控制下保持竞争力；ES更新更大、诱导更广泛离任务KL漂移，GRPO更新更小更局部；两者解线性连接无损失障碍，尽管更新方向几乎正交

Conclusion: 无梯度和基于梯度的微调可达到相似准确率但几何结构不同的解，ES在弱信息方向上积累大离任务移动但仍能取得足够任务进展，这对遗忘和知识保留有重要影响
...
TL;DR: 该论文提出REPAIR方法，通过贝叶斯最优重排序解决长尾分类问题，将残差校正分解为类别固定项和输入依赖的成对项，解释何时需要成对校正。


<details>
  <summary>Details</summary>
Motivation: 长尾分类中，现有后处理方法（如logit adjustment）使用固定的类别偏移来校正模型对频繁类的偏好，但这种固定偏移无法适应不同输入间的变化。需要研究何时以及如何通过输入依赖的校正来恢复贝叶斯最优排序。

Method: 提出REPAIR方法：基于基础模型top-k候选列表进行贝叶斯最优重排序。将最优得分与基础得分之间的残差分解为：1）类别固定项（每个类别内恒定）；2）成对项（依赖于输入和竞争标签）。REPAIR结合收缩稳定的类别项和由候选列表竞争特征驱动的线性成对项，形成轻量级后处理重排序器。

Result: 在五个基准测试（图像分类、物种识别、场景识别、罕见疾病诊断）上的实验证实：分解理论解释了何时成对校正能提升性能，何时仅类别校正就足够。当相同标签对在不同上下文中引发不相容的排序约束时，固定偏移无法恢复贝叶斯最优排序。

Conclusion: 长尾分类的残差校正应分解为类别固定项和输入依赖的成对项。REPAIR方法通过这种分解实现了轻量级后处理重排序，理论分析解释了何时需要成对校正，为长尾分类提供了新的理论框架和实用解决方案。
...
TL;DR: CCN框架通过基于用户状态和对话上下文的学习标量信号来调节响应生成和候选选择，在保持支持性的同时改善自主性保护


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法主要优化帮助性和无害性，但忽略了关系风险如依赖强化、过度保护或强制指导，需要平衡帮助性与用户自主性保护

Method: 提出关怀条件神经调节(CCN)框架，将用户状态和对话上下文转化为标量信号来调节响应生成；定义奖励自主性支持和帮助性、惩罚依赖和强制的效用函数；结合关怀条件候选生成和基于效用的重排序

Result: 在关系失败模式基准测试中，CCN比监督微调提高自主性保护效用+0.25，比偏好优化基线提高+0.07，同时保持可比的支持性；人类评估和零样本迁移到真实情感支持对话显示与自动指标方向一致
...
### [20] [Thinking While Listening: Fast-Slow Recurrence for Long-Horizon Sequential Modeling](https://arxiv.org/abs/2604.01577)
*Shota Takashiro,Masanori Koyama,Takeru Miyato,Yusuke Iwasawa,Yutaka Matsuo,Kohei Hayashi*

Main category: cs.LG

TL;DR: 将潜在循环建模扩展到顺序输入流，通过交错快速循环潜在更新和慢速观测更新，学习稳定内部结构，改善长时程表示和泛化能力


<details>
  <summary>Details</summary>
Motivation: 现有序列模型（如LSTM、状态空间模型、Transformer变体）在处理长序列时难以维持稳定的内部结构表示，限制了在强化学习和算法任务中的分布外泛化能力

Method: 提出交错更新机制：在慢速观测更新之间插入快速循环潜在更新，利用自组织能力建立稳定且演化的内部结构，保持长期连贯和聚类表示

Result: 相比LSTM、状态空间模型和Transformer变体等序列基线，在强化学习和算法任务中表现出更好的分布外泛化能力

Conclusion: 通过交错快速循环潜在更新和慢速观测更新的机制，能够学习稳定演化的内部结构，改善长时程表示保持和泛化性能
...
### [21] [Optimizing EEG Graph Structure for Seizure Detection: An Information Bottleneck and Self-Supervised Learning Approach](https://arxiv.org/abs/2604.01595)
...
<details>
  <summary>Details</summary>
Motivation: 现有EEG癫痫检测方法构建动态图时很少考虑EEG数据的噪声特性，导致图包含冗余或任务无关的连接，影响模型性能

Method: 提出IRENE框架：1) 基于信息瓶颈原理学习去噪的动态图结构；2) 使用图掩码自编码器进行自监督学习，基于动态图上下文重建掩码的EEG信号；3) 联合学习动态图结构和可解释的时空表示

Result: 在基准EEG数据集上的实验表明，该方法在癫痫检测方面优于现有最先进基线，并提供对癫痫传播的临床有意义见解

Conclusion: IRENE通过信息瓶颈指导的图结构学习和自监督表示学习，有效解决了EEG癫痫检测中的噪声、标签稀缺和患者间变异性等核心挑战
...
TL;DR: I-PPO通过梯度影响分数筛选训练数据，过滤有害推理轨迹，提升PPO训练效率和模型性能


<details>
  <summary>Details</summary>
Motivation: 传统PPO算法假设所有生成轨迹都有益，但实际上包含大量噪声和不忠实推理，这会降低模型性能并减慢训练速度

Method: 提出I-PPO框架，在RL后训练循环中集成数据归因，通过基于梯度的近似计算每个轨迹的影响分数，识别并消除与验证梯度反对齐的轨迹

Result: I-PPO在实验中持续优于SFT和PPO基线，过滤过程起到内在早停机制作用，加速训练效率并有效减少不忠实CoT推理

Conclusion: 通过影响指导的数据筛选可以显著提升RL训练效果，减少有害推理轨迹对模型性能的负面影响
...
TL;DR: 论文提出对比上下文训练方法，通过混合相似和随机示例来促进上下文学习与权重学习的协同发展，避免模型退化到纯复制或纯权重学习模式。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然同时具备上下文学习和权重学习能力，但标准的任务特定微调往往会削弱上下文学习能力。现有研究表明上下文学习的出现依赖于任务多样性和训练时长等因素，但输入目标与上下文示例之间的相似性结构的作用尚未充分研究。

Method: 提出对比上下文训练方法，强制两种对比：(1) 在单个上下文中混合相似和随机示例，以演化出正确的上下文学习形式；(2) 在不同上下文中使用不同相似度等级的示例，以演化出上下文学习与权重学习的混合模式。通过理论分析最小模型验证对比的重要性。

Result: 在四个大语言模型和多个任务上的广泛实证评估表明，对比上下文训练能产生稳定的上下文学习-权重学习混合模式，避免模型退化到纯上下文学习、纯权重学习或纯标签复制模式。诊断探针确认了这种混合模式的稳定性。

Conclusion: 目标输入与上下文示例之间的相似性结构对上下文学习的演化至关重要。对比上下文训练通过强制相似性和随机性的对比，能够协同发展上下文学习和权重学习能力，使模型能够根据上下文相关性在这两种学习模式之间灵活切换。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散语言模型的混合专家系统沿用了自回归模型的令牌选择路由机制，导致负载不均衡和计算分配僵化，需要更适合扩散模型特性的路由方案。

Method: 采用专家选择路由替代令牌选择路由，引入时间步依赖的专家容量分配策略，根据去噪步骤动态调整专家分配，并将更多容量分配给低掩码率步骤。

Result: 专家选择路由在扩散语言模型中实现了确定性负载均衡，带来更高的吞吐量和更快的收敛速度；低掩码率步骤的令牌学习效率高出一个数量级，集中计算资源可获得最大边际回报；现有预训练模型仅通过替换路由器即可升级为专家选择路由。

Conclusion: 专家选择路由是扩散语言模型混合专家系统的优越范式，证明了扩散模型中的计算可以作为自适应策略而非固定架构常数。
...
TL;DR: CRIT是一个新的跨模态多跳推理数据集和基准，通过图基自动流程生成复杂推理任务，解决现有基准中模态互补推理不足的问题


<details>
  <summary>Details</summary>
Motivation: 现实世界推理需要跨模态的多跳推理，但现有多模态基准通常依赖单一图像或图像集，答案可从单一模态推断，训练数据中交错的图像-文本内容也很少强制互补的多跳推理，导致视觉语言模型经常产生幻觉和缺乏视觉证据基础的推理轨迹

Method: 引入CRIT数据集和基准，采用基于图的自动流程生成复杂的跨模态推理任务，涵盖自然图像、视频和文本丰富来源等多个领域，并包含手动验证的测试集以确保可靠评估

Result: 实验表明，即使是先进模型在此类推理任务上也表现不佳，而在CRIT上训练的模型在跨模态多跳推理方面取得显著提升，包括在SPIQA和其他标准多模态基准上的强劲改进
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中训练集和测试集的标签分布往往不同（如医疗诊断、欺诈检测、社交媒体分类），传统监督学习假设相同分布不成立。现有标签偏移估计方法要么基于验证集混淆矩阵进行矩匹配，要么使用EM算法最大化新数据似然，存在局限性

Method: 提出后验标签偏移估计方法，增量更新每个样本的先验概率来调整后验分布。该方法基于现代概率分类器的直观假设，相比其他方法依赖更弱的校准概念，可应用于任何黑盒概率分类器

Result: 在CIFAR-10和MNIST数据集上的实验表明，该方法在不同校准程度和不同强度标签偏移下，始终优于当前最先进的基于最大似然的方法

Conclusion: 提出了一种通用且有效的标签偏移估计方法，适用于现实世界标签分布变化的场景，为黑盒概率分类器提供了灵活的后处理解决方案
...
<details>
  <summary>Details</summary>
Motivation: 虽然GANs已广泛用于增强EEG数据，但尚不清楚合成EEG是否保留了基于转换分析所需的底层动态结构。需要评估合成EEG是否保持分布几何结构，以支持基于能量的认知状态转换建模。

Method: 使用SBP衍生的传输成本作为度量，比较从真实和合成EEG（在Stroop任务中收集）推导出的转换能量。在群体和参与者层面进行分析，验证合成EEG是否保留SBP建模所需的转换结构。

Result: 真实和合成EEG在Stroop任务中推导出的转换能量在群体和参与者层面分析中表现出强烈一致性，表明合成EEG保留了SBP建模所需的转换结构。

Conclusion: 合成EEG保留了SBP建模所需的转换结构，可用于数据高效的神经自适应系统。研究进一步提出了一个框架，其中SBP衍生的认知能量作为自适应人机系统的控制信号，支持实时调整系统行为以响应用户认知和情感状态。
...
TL;DR: MiCA是一种新颖的参数高效微调方法，专注于适应模型表示中未充分利用的子空间，通过约束参数更新到次要奇异向量方向，在知识获取方面比LoRA提升5.9倍，参数占用仅为6-60%。
...
Method: MiCA使用奇异值分解识别与最小奇异值相关的次要奇异向量子空间，在微调过程中约束参数更新到这些方向，从而专注于适应模型表示中未充分利用的部分。

Result: 在优化的训练超参数下，MiCA在知识获取方面比LoRA提升高达5.9倍，同时参数占用仅为LoRA的6-60%，表明次要奇异方向的约束适应提供了更高效的知识集成。

Conclusion: 约束适应到次要奇异方向为预训练语言模型集成新知识提供了更高效和稳定的机制，MiCA方法在参数效率和性能方面均优于传统方法。
...
### [29] [Transformer self-attention encoder-decoder with multimodal deep learning for response time series forecasting and digital twin support in wind structural health monitoring](https://arxiv.org/abs/2604.01712)
*Feiyu Zhou,Marios Impraimakis*

Main category: cs.LG

TL;DR: 本文提出了一种基于Transformer的桥梁结构健康监测数字孪生框架，用于风致结构响应预测和异常检测，在挪威哈当厄尔大桥实测数据上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 风致结构响应预测面临环境条件变化带来的不确定性，传统方法需要假设风的平稳性和结构正常振动行为，难以准确区分正常与异常振动。需要开发能够适应环境变化、无需先验假设的智能监测方法。

Method: 提出基于Transformer的框架：1）利用系统时间特性训练预测模型；2）将振动预测值与实测值比较以检测大偏差；3）将识别出的异常情况作为结构变化的早期预警指标。该方法无需假设风的平稳性或结构正常振动行为。

Result: 在挪威科技大学监测的哈当厄尔大桥实测数据上验证，该方法在真实条件下准确捕捉结构行为，适应系统激励变化。基于Transformer的数字孪生组件在响应预测方面优于传统方法。

Conclusion: 基于Transformer的数字孪生组件具有作为下一代韧性基础设施管理工具的潜力，支持连续学习和自适应监测，能够适应系统生命周期内的时间特性变化。
...
### [30] [MATA-Former & SIICU: Semantic Aware Temporal Alignment for High-Fidelity ICU Risk Prediction](https://arxiv.org/abs/2604.01727)
*Zhichong Zheng,Xiaohang Nie,Xueqi Wang,Yuanjin Zhao,Haitao Zhang,Yichao Tang*

Main category: cs.LG

TL;DR: MATA-Former：基于医学语义感知的Transformer模型，通过事件语义动态参数化注意力权重，结合Plateau-Gaussian软标签方法，用于临床风险轨迹预测


<details>
  <summary>Details</summary>
Motivation: 当前临床风险预测方法存在两个主要问题：1）依赖粗粒度的二元监督和物理时间戳，而非病理依赖关系；2）预测建模与临床逻辑不对齐。需要开发能够捕捉内在病理依赖而非仅时间邻近性的方法。
...
Result: 在SIICU（新构建数据集，包含50.6万+事件，具有专家验证的细粒度标注）和MIMIC-IV数据集上评估，该框架在捕捉文本密集型、不规则临床时间序列风险方面表现出优越效果和鲁棒泛化能力。

Conclusion: 通过将事件语义融入注意力机制和采用连续风险轨迹建模，MATA-Former框架能够更好地对齐临床逻辑，在临床风险预测任务中实现更准确、更符合病理依赖关系的预测。
...
### [31] [Koopman-Based Nonlinear Identification and Adaptive Control of a Turbofan Engine](https://arxiv.org/abs/2604.01730)
*David Grasev*

Main category: cs.LG

TL;DR: 该研究开发了基于Koopman算子的双转子涡扇发动机多变量控制方法，提出元启发式扩展动态模态分解构建单一Koopman模型，并比较了自适应Koopman模型预测控制器与反馈线性化控制器的性能。


<details>
  <summary>Details</summary>
Motivation: 涡扇发动机控制面临多变量、非线性挑战，传统方法难以同时准确预测转子转速和发动机压力比。研究旨在开发基于Koopman算子的控制框架，实现单一模型适应多种控制目标，提高控制鲁棒性和灵活性。

Method: 1. 建立基于物理的部件级模型生成训练数据；2. 开发元启发式扩展动态模态分解方法，设计成本函数准确捕捉转子转速动态和发动机压力比；3. 构建时变Koopman模型；4. 开发两种控制器：带扰动观测器的自适应Koopman模型预测控制器和作为基准的Koopman反馈线性化控制器；5. 在两种控制策略（转子转速配置和EPR控制）下评估控制器性能。

Result: 1. 提出的辨识方法能准确预测转子转速和发动机压力比；2. Koopman模型可在不同控制公式中灵活重用；3. 两种控制策略在稳态条件下性能相当；4. 在变化飞行条件下，AKMPC比K-FBLC表现出更优的鲁棒性，能补偿模型失配；5. EPR控制策略改善了推力响应。

Conclusion: 研究证明了Koopman基控制在涡扇发动机控制中的适用性，展示了AKMPC框架在鲁棒控制方面的优势。单一Koopman模型能灵活适应多种控制目标，自适应模型预测控制器在变化飞行条件下具有更好的鲁棒性，为涡扇发动机控制提供了有效解决方案。
...
### [32] [FourierMoE: Fourier Mixture-of-Experts Adaptation of Large Language Models](https://arxiv.org/abs/2604.01762)
*Juyong Jiang,Fan Wang,Hong Qi,Sunghun Kim,Jing Tang*

Main category: cs.LG

TL;DR: FourierMoE：一种在谱域进行频率感知适配的PEFT方法，通过MoE架构与逆离散傅里叶变换结合，实现多任务微调中的参数高效优化


<details>
  <summary>Details</summary>
Motivation: 传统PEFT方法在多任务微调中存在任务干扰和表示不足问题，现有MoE方法主要在空间域操作，存在结构冗余和参数开销。研究发现不同任务具有不同的频率能量分布，LLM层对频率敏感度存在异质性，这启发了在谱域进行适配的思路。

Method: 提出FourierMoE，将MoE架构与逆离散傅里叶变换(IDFT)结合进行频率感知适配。使用频率自适应路由器将token分发到专门处理不同频段的专家，每个专家学习一组共轭对称的复系数，保留完整的相位和幅度信息，理论上保证通过IDFT无损重构为实值空间权重。

Result: 在28个基准测试、多种模型架构和规模上的广泛评估表明，FourierMoE在单任务和多任务设置中均优于竞争基线，同时使用显著更少的可训练参数。

Conclusion: 谱域专家适配是一种有效且参数高效的LLM微调范式，FourierMoE通过频率感知的MoE架构解决了多任务PEFT中的任务干扰和表示不足问题。
...
### [33] [Bridging Deep Learning and Integer Linear Programming: A Predictive-to-Prescriptive Framework for Supply Chain Analytics](https://arxiv.org/abs/2604.01775)
*Khai Banh Nghiep,Duc Nguyen Minh,Lan Hoang Thi*

Main category: cs.LG

TL;DR: 该研究提出一个三阶段分析框架，结合需求预测与运营优化，使用N-BEATS和N-HiTS深度学习模型预测零售需求，并通过整数线性规划生成成本最优的配送计划。
...
Method: 三阶段分析框架：1) 探索性数据分析，分析180,519笔交易数据；2) 比较N-BEATS MSTL统计模型和N-HiTS深度学习模型的预测性能；3) 选择最优模型预测未来4周需求，通过整数线性规划优化配送计划。

Result: N-HiTS和N-BEATS深度学习模型大幅超越统计基准，N-BEATS被选为最优模型（预测误差最低），预测未来4周需求为1918单位，整数线性规划生成了可行且成本最优的配送方案。
...
<details>
  <summary>Details</summary>
Motivation: 现代过程工业中，数据驱动模型对实时监控至关重要，但关键性能指标难以直接测量。虽然准确预测很重要，但可靠的不确定性量化对于安全、可靠性和决策同样关键，而现有数据驱动方法在这方面仍面临重大挑战

Method: 引入基于扩散的后验采样框架，通过忠实后验采样自然产生良好校准的预测不确定性，无需后校准
...
<details>
  <summary>Details</summary>
Motivation: 现有谱图对比学习方法在处理同质性和异质性混合图时面临谱困境：高频信号对编码异质性至关重要，但理论分析表明其在谱集中扰动下具有显著更高的方差。全局（节点无关）谱融合策略在具有分离节点频率偏好的混合图上被证明是次优的。

Method: 提出ASPECT框架，通过可靠性感知的谱门控机制解决谱困境。该框架被表述为极小极大博弈，采用节点级门控机制，基于频率通道在对抗性攻击下的稳定性动态重新加权频率通道。对抗器通过Rayleigh商惩罚明确针对谱能量分布，迫使编码器学习既具有结构区分性又具有谱鲁棒性的表示。

Result: 在9个基准测试中的8个上实现了新的最先进性能，有效地将有意义的结构异质性与偶然噪声解耦。

Conclusion: ASPECT通过节点级可靠性感知谱融合解决了谱图对比学习中的基本谱困境，为处理混合图提供了一种理论上有保证且实际有效的解决方案。
...
TL;DR: 提出利用专家分歧生成训练目标的新方法，通过两集成模型分别估计认知和偶然不确定性，在医疗AI中提升不确定性估计质量9%-50%


<details>
  <summary>Details</summary>
Motivation: 医疗AI系统作为第二意见系统可加速工作流程并提高诊断准确性，但AI错误的不可预测性带来重大挑战，尤其在医疗环境中错误可能造成严重后果。现有不确定性估计方法在量化偶然不确定性（源于数据模糊性和噪声）方面仍有限制。

Method: 提出利用专家响应中的分歧生成训练目标的新方法，这些目标与标准数据标签结合，通过两集成方法（及其轻量变体）根据总方差定律分别估计不确定性的两个组成部分。

Result: 在二元图像分类、二元和多类图像分割以及多项选择题回答任务上验证方法，实验表明结合专家知识可将不确定性估计质量提升9%到50%，具体取决于任务类型。

Conclusion: 专家知识作为信息源对于构建医疗应用中的风险感知AI系统具有不可估量的价值，所提方法能有效提升不确定性估计质量，增强医疗AI系统的可靠性和安全性。
...
TL;DR: PROWL提出了一种新的PAC-Bayesian方法，用于在奖励不确定性的情况下学习稳健的个体化治疗规则，通过构建保守奖励和策略依赖的下界来保证有限样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于结果加权学习(OWL)的个体化治疗规则(ITR)估计方法依赖于可能包含噪声或过于乐观的观察奖励，忽略奖励不确定性会导致选择具有虚高表现的政策，且现有OWL框架缺乏将这种不确定性系统嵌入学习目标的有限样本保证。

Method: 提出PAC-Bayesian Reward-Certified Outcome Weighted Learning (PROWL)：1) 给定单侧不确定性证书，构建保守奖励和严格策略依赖的真实期望值下界；2) 证明精确的认证约简，将稳健策略学习转化为统一的、无需分割的成本敏感分类任务；3) 推导随机ITR的非渐近PAC-Bayes下界，并证明最大化该下界的最优后验由广义贝叶斯更新精确表征；4) 引入完全自动化的基于边界的校准程序解决广义贝叶斯推理中的学习率选择问题；5) 提出Fisher一致的认证铰链替代函数用于高效优化。

Result: 实验表明，在严重奖励不确定性的情况下，PROWL相比标准的ITR估计方法，在估计稳健、高价值治疗机制方面取得了改进。

Conclusion: PROWL为奖励不确定性下的个体化治疗规则学习提供了具有有限样本保证的PAC-Bayesian框架，通过理论认证约简和自动化校准机制，实现了对稳健高价值治疗机制的改进估计。
...
TL;DR: annbatch是一个原生anndata的mini-batch加载器，支持直接在磁盘数据集上进行out-of-core训练，显著提升生物数据加载吞吐量


<details>
  <summary>Details</summary>
Motivation: 生物数据集规模已超过系统内存容量，数据访问而非模型计算成为机器学习训练的主要瓶颈。生物领域广泛使用的社区数据格式需要支持异构元数据、稀疏和密集检测以及现有计算生态系统中的下游分析

Method: 开发annbatch，一个原生anndata的mini-batch加载器，支持直接在磁盘数据集上进行out-of-core训练，保持与scverse生态系统的完全兼容性
...
TL;DR: LSCP是一种自门控后训练框架，通过异常高损失检测、自问答链生成和基于置信度的AdamW β₂调整，实现自主知识获取，防止死记硬背并保护相邻知识。


<details>
  <summary>Details</summary>
Motivation: 解决模型在微调时出现的死记硬背问题，实现语义学习而非机械记忆；同时保护已有知识不被新内容污染，模拟生物记忆巩固过程。

Method: 1) 检测异常高损失的段落；2) 生成问答链让模型自我验证知识缺口；3) 根据置信度深度k调整AdamW的β₂参数：β₂ = 0.999·r^k；4) 整个学习强度由单一参数r控制。

Result: 标准微调产生死记硬背（扰动差距11.6±0.2倍基线），而所有LSCP条件实现语义学习（2.7-3.0倍）。r=1.0条件证明训练数据格式（而非β₂门控）是防止记忆的主要机制；门控保护相邻知识免受污染（r=0.98时相邻问题准确率93±7% vs 90%基线）。

Conclusion: LSCP框架通过自门控机制实现自主知识获取，促进语义学习而非机械记忆，保护已有知识完整性，并模拟生物记忆巩固过程，随着学习进展自动收敛到标准优化器。
...
TL;DR: WAV框架通过将状态预测分解为状态合理性和动作可达性两个因子，利用数据不对称性实现世界模型的自验证和自改进，显著提升样本效率和策略性能。


<details>
  <summary>Details</summary>
Motivation: 通用世界模型需要比策略学习更广泛的鲁棒性，必须对次优动作也能可靠预测，但现有动作标记交互数据往往覆盖不足，导致在未充分探索区域预测失败。

Method: 将动作条件状态预测分解为状态合理性和动作可达性；利用视频语料库生成多样化子目标；构建稀疏逆模型从状态特征子集推断动作；通过子目标生成、动作推断和前向推演的循环一致性实现验证。

Result: 在MiniGrid、RoboMimic和ManiSkill的9个任务中，方法达到2倍样本效率提升，下游策略性能提高18%，在未充分探索区域表现显著优于现有方法。

Conclusion: 通过分解验证和利用数据不对称性，WAV框架能有效识别预测错误并自我改进，为构建更鲁棒的通用世界模型提供了新途径。
...
TL;DR: Apriel-Reasoner是一个15B参数的开源推理模型，通过多领域RL后训练在数学、代码生成、指令遵循、逻辑谜题和函数调用五个领域进行优化，采用自适应领域采样和难度感知长度惩罚机制，在保持推理准确性的同时显著缩短推理轨迹长度。


<details>
  <summary>Details</summary>
Motivation: 现有前沿开源模型虽然广泛采用基于可验证奖励的强化学习进行通用推理训练，但其训练配方和领域混合策略通常不公开。跨领域联合优化面临重大挑战：不同领域在轨迹长度、问题难度和样本效率上差异巨大。此外，长思维链会增加推理成本和延迟，实际部署需要高效性。

Method: 1) 在Apriel-Base（15B参数开源LLM）上进行完全可复现的多领域RL后训练；2) 引入自适应领域采样机制，在异构轨迹动态下保持目标领域比例；3) 提出难度感知的标准长度惩罚扩展，无需额外训练开销，鼓励困难问题使用更长推理，简单问题使用更短轨迹；4) 使用严格的16K token输出预算进行训练。

Result: Apriel-Reasoner在推理时可泛化到32K token，在AIME 2025、GPQA、MMLU-Pro和LiveCodeBench上优于Apriel-Base，同时产生30-50%更短的推理轨迹。在相似规模的开源模型中达到同等性能，但token成本更低，从而推进了准确性与token预算的帕累托前沿。

Conclusion: Apriel-Reasoner通过自适应领域采样和难度感知长度惩罚机制，有效解决了多领域RL训练中的异质性问题，在保持推理准确性的同时显著提高了效率。该方法为构建高效实用的通用推理模型提供了可复现的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的主动学习回归方法在计算样本间距离时没有考虑不同特征的重要性，导致样本选择不够优化。代表性（representativeness）和多样性（diversity）是主动学习中的重要考虑因素，但现有方法在距离计算中未能有效利用特征权重信息。

Method: 提出了三种特征加权的单任务主动学习回归方法和两种特征加权的多任务主动学习回归方法。核心思想是：利用少量已标记样本训练的岭回归系数作为特征权重，在计算样本间距离时对相应特征进行加权。这种方法易于实现，可以增强现有主动学习回归方法的性能。

Result: 实验表明，这种易于实现的增强方法几乎总是能够提升四种现有主动学习回归方法的性能，无论是在单任务还是多任务回归问题中。特征加权策略还可以轻松扩展到流式主动学习和分类算法。

Conclusion: 通过将岭回归系数作为特征权重引入样本间距离计算，可以有效提升主动学习回归的性能。这种特征加权策略简单易行，具有很好的扩展性，可以应用于更广泛的主动学习场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分解的压缩方法存在局限性：要么只优化原始输入而忽略上游压缩引起的分布偏移，导致误差传播；要么只依赖偏移后的输入而可能偏离原始输出。需要一种能同时考虑两者的方法来实现更好的压缩效果。

Method: 1) 同时考虑原始输入和分布偏移，将压缩层锚定到原始输出；2) 在单个层压缩基础上，对每个Transformer块进行端到端细化，最小化块级输出失真；3) 允许压缩层共同补偿累积误差，找到保持功能等效的低秩近似。

Result: 在大语言模型上的实验表明，该方法在各种压缩比下始终优于现有的SVD基线方法。在激进压缩预算下优势更加明显，而竞争方法性能大幅下降或完全崩溃。

Conclusion: 该方法为高效、大规模模型部署提供了实用解决方案，通过同时建模原始输出和输入分布偏移，实现了更好的功能等效性，特别是在高压缩比下表现出色。
...
TL;DR: 提出基于拍卖机制的模块化多目标强化学习框架，通过自私局部策略竞价执行动作，实现运行时目标动态变化的适应性
...
Method: 模块化框架：每个目标对应自私局部策略，通过拍卖机制协调——策略根据状态紧迫性竞价，最高出价者选择动作；将问题转化为一般和博弈，策略同时优化自身目标并学习校准出价；使用PPO并发训练策略

Result: 在Atari Assault和网格世界路径规划任务上评估，相比PPO训练的单一策略获得显著更好的性能

Conclusion: 提出的拍卖机制框架能够有效处理动态多目标强化学习问题，实现运行时自适应，性能优于传统方法
...
<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法如NOTEARS和DAG-GNN在高维数据中面临可扩展性和稳定性问题，特别是在特征-样本不平衡的情况下。需要一种更高效、更稳定的方法来建模观测数据中的因果依赖关系。

Method: 提出Denoising Diffusion Causal Discovery (DDCD)框架：1) 利用扩散模型的去噪分数匹配目标平滑梯度以加速收敛；2) 提出自适应k-hop无环约束，避免矩阵求逆，改进运行时性能；3) 不同于生成式扩散模型，DDCD使用反向去噪过程推断参数化的因果结构而非生成数据。

Result: 在合成基准数据上展示了DDCD的竞争性能，并在两个真实世界案例中进行了定性分析，证明了方法的实用性。

Conclusion: DDCD通过结合扩散模型的梯度平滑特性和自适应无环约束，提供了一种更高效、更稳定的因果发现方法，在高维数据中具有实际应用价值。
...
TL;DR: 针对非平稳动态系统的持续模型强化学习，提出自适应数据缓冲机制以处理时变动态


<details>
  <summary>Details</summary>
Motivation: 现实世界系统常因漂移、磨损或运行条件变化而具有时变动态，传统基于学习的控制方法假设静态系统动态不适用于此类场景

Method: 采用高斯过程动态模型，在频繁变化预算假设下分析问题，提出具有自适应数据缓冲机制的乐观模型强化学习算法

Result: 分析表明持续非平稳性需要限制过时数据影响以保持校准不确定性和有意义的动态遗憾保证，算法在具有非平稳动态的连续控制基准上表现改进

Conclusion: 处理时变动态需要显式管理数据时效性，自适应数据缓冲机制是应对持续非平稳性的有效策略
...
TL;DR: SRPO统一了GRPO和SDPO的优势，通过样本路由机制将正确样本分配给GRPO进行奖励对齐强化学习，将失败样本分配给SDPO进行针对性的logit级修正，解决了SDPO长期训练不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在局限性：GRPO采用粗粒度的信用分配，对失败rollout进行统一惩罚，缺乏token级别的针对性；SDPO虽然提供更密集的logit级监督，能实现快速早期改进，但在长期训练中经常崩溃。需要解决SDPO后期不稳定的内在缺陷。

Method: 提出SRPO框架：1) 样本路由机制：将正确样本路由到GRPO进行奖励对齐的强化学习，将失败样本路由到SDPO进行针对性的logit级修正；2) 熵感知动态加权机制：抑制高熵、不可靠的蒸馏目标，强调置信度高的目标。

Result: 在五个基准测试和两种模型规模上，SRPO同时实现了SDPO的快速早期改进和GRPO的长期稳定性。在Qwen3-8B上，五个基准测试的平均性能比GRPO提升3.4%，比SDPO提升6.3%，同时产生适度的响应长度，每步计算成本降低高达17.2%。

Conclusion: SRPO成功解决了SDPO的长期训练不稳定问题，统一了GRPO和SDPO的优势，实现了快速收敛和稳定训练，在性能、效率和响应长度方面均优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 在低精度推理的小型Transformer模型中，softmax成为计算瓶颈，其指数运算和归一化操作带来显著开销。AMD Versal AI Engines平台的现有实现使用bfloat16或查找表进行指数运算，无法充分利用平台的高吞吐量整数向量处理单元。

Method: 提出HCCS方法：使用裁剪线性映射替代指数函数，包含轻量级校准参数，基于代表性数据集离线优化，为每个注意力头单独校准以保持统计特性。该方法自然映射到AI Engines的int8乘累加单元。

Result: HCCS在AMD AI Engines上显著超过其他参考实现的速度性能，在小型或重度量化MHA工作负载上，经过量化感知重训练后保持竞争性任务精度。这是首个针对AMD AI Engines的int8优化softmax替代方案。

Conclusion: HCCS为Transformer模型的softmax计算瓶颈提供了有效的硬件优化解决方案，特别适合AMD Versal AI Engines平台，通过整数运算替代浮点指数计算，在保持精度的同时大幅提升推理速度。
...
TL;DR: 提出go-mHC方法，基于广义正交随机矩阵理论，实现双随机矩阵的精确参数化，复杂度为O(d³)，通过超参数s在计算效率和完全表达性之间连续插值。


<details>
  <summary>Details</summary>
Motivation: 双随机矩阵能够实现残差流之间的学习混合，但精确高效地参数化双随机矩阵集（Birkhoff多面体）仍然是一个开放挑战。现有精确方法随流数量d呈阶乘级扩展，而Kronecker分解方法效率高但表达能力有限。

Method: 基于广义正交随机矩阵理论提出新颖的精确参数化方法，复杂度为O(d³)。该方法引入单个超参数s，可在计算高效边界和完全表达的Birkhoff多面体之间连续插值。在流形约束超连接(mHC)框架基础上实例化为go-mHC方法，可与Kronecker分解方法自然组合。

Result: 谱分析表明go-mHC比Kronecker分解基线更完整地填充Birkhoff多面体。在合成流混合任务中，go-mHC达到最小理论损失，收敛速度提升高达10倍。在3000万参数GPT风格语言模型中验证了方法的有效性。

Conclusion: go-mHC的表达能力、效率和精确性为将d作为模型容量的新维度进行扩展提供了实用途径。该方法在相似FLOP成本下显著恢复表达能力，实现了双随机矩阵参数化的突破。
...
TL;DR: BCR是一种单阶段训练范式，通过让模型在共享上下文中同时解决N个问题，仅以每个实例的准确性为奖励，实现高效的推理，减少15.8%-62.6%的token使用同时保持或提高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型思维链推理方法存在token消耗过多、推理成本高的问题。现有的效率方法如显式长度惩罚、难度估计或多阶段课程要么会降低推理质量，要么需要复杂的训练流程。

Method: 提出批量上下文强化学习（BCR），这是一种极简主义的单阶段训练范式：训练模型在共享上下文窗口中同时解决N个问题，仅以每个实例的准确性为奖励。这种表述创建了一个隐式的token预算。

Result: 1) 发现新的任务缩放定律：随着推理时并发问题数N增加，每个问题的token使用单调减少，而准确性下降远慢于基线；2) BCR挑战传统的准确性-效率权衡，在标准单问题推理中实现"免费午餐"现象，在1.5B和4B模型上减少15.8%-62.6%的token使用同时保持或提高准确性；3) 定性分析显示模型自主消除冗余元认知循环；4) 隐式预算约束成功规避显式长度惩罚的对抗梯度和灾难性优化崩溃。

Conclusion: BCR证明了简单的结构激励可以解锁大语言模型中潜在的高密度推理能力，提供了一种高度稳定、基于约束的长度控制替代方案，具有实际应用价值。
