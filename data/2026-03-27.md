...
Motivation: 现有研究主要关注静态设置，忽略了多模态知识的动态获取和融合。具体问题包括：1) 知识获取层面难以学习不同模态（特别是动态结构模态）的时间敏感信息；2) 知识融合层面难以学习演化的多模态融合特征，静态共注意力机制无法捕捉不同模态对历史事件变化的贡献

Method: DyMRL采用动态多空间表示学习方法：1) 将欧几里得、双曲和复数空间的时间特定结构特征整合到关系消息传递框架中，学习深度表示；2) 采用先进的双重融合-演化注意力机制，以对称方式在不同时间戳为不同模态分配动态学习权重；3) 利用预训练模型赋予时间敏感的视觉和语言智能

Result: 构建了四个多模态时序知识图谱基准数据集进行实验。广泛实验表明，DyMRL在事件预测性能上优于最先进的动态单模态和静态多模态基线方法

Conclusion: DyMRL通过动态获取和融合多模态时序知识，有效解决了现有方法在时间敏感信息学习和演化融合特征捕获方面的局限性，在事件预测任务中表现出优越性能
...
Conclusion: 通过单次尝试经验反思提取可迁移启发式规则，能够实现有效的智能体自我改进，为LLM智能体在专业环境中的快速适应提供了新方法
...
<details>
  <summary>Details</summary>
Motivation: 本文的动机在于探索随机裁剪这一常见数据增强技术在差分隐私训练中未被充分研究的作用。当图像中的敏感内容（如人脸、车牌）空间局部化时，随机裁剪可以概率性地将这些内容排除在模型输入之外，从而为差分隐私训练提供额外的随机性来源。

Method: 作者引入了一种针对视觉数据的补丁级相邻关系，并推导了将随机裁剪与差分隐私随机梯度下降（DP-SGD）结合时的严格隐私界限。该方法量化了补丁包含概率，并展示了其如何与小批量采样组合以降低有效采样率。

Result: 理论分析表明，补丁级隐私放大改善了隐私-效用权衡。实证验证在多个分割架构和数据集上证实了这一点，显示对齐隐私计算与领域结构和现有随机性源可以在不增加成本的情况下提供更强的隐私保证。

Conclusion: 随机裁剪的固有随机性可以放大差分隐私效果，这为计算机视觉中的隐私保护训练提供了一种无需修改模型架构或训练程序的新方法。通过将隐私计算与领域结构对齐，可以利用现有随机性源获得更强的隐私保证。
...
<details>
  <summary>Details</summary>
Motivation: 多步工具编排中LLMs需要按正确顺序调用多个依赖API并传递中间输出，现有模型在完整序列执行上经常失败，参数值错误占失败的主要部分。现有训练环境专注于简单的单轮函数调用和模拟数据，二元奖励无法提供部分正确性的信号。

Method: 1) 构建基于大规模真实API响应缓存的强化学习环境，支持数据合成管道，可采样具有可控复杂度的有效多步编排轨迹，生成效率显著高于无约束方法；2) 提出分级奖励设计，将正确性分解为原子有效性（单个函数调用在递增粒度上的正确性）和编排（正确的工具排序和依赖关系尊重）。

Result: 在ComplexFuncBench上，该方法在轮次准确性方面展示了显著改进。消融研究证实两个奖励组件都是必需的：单独使用任一组件的性能都会显著下降。

Conclusion: 该框架通过真实API响应缓存环境和分级奖励设计，有效解决了多步工具编排的训练挑战，为LLMs处理复杂工作流提供了更有效的训练方法。
...
TL;DR: ACOF是一个用于模拟电路尺寸优化的演员-评论家框架，通过分离提案和评估角色，将设计者的判断引入优化循环，相比传统黑盒搜索方法显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 模拟设计面临的主要挑战是：即使对器件尺寸或偏置进行微小改动也需要昂贵的仿真周期，而高质量解决方案通常只占据巨大搜索空间中非常狭窄的部分。现有优化器虽然减轻了部分负担，但缺乏设计者在决定下一步搜索方向时所使用的那种判断能力。

Method: 提出了演员-评论家优化框架（ACOF），将优化过程分解为提案和评估两个角色：演员负责提出设计空间中有前景的区域，评论家则评估这些选择、强制执行设计合法性，并在进展受阻时重新定向搜索。该框架保持了与标准基于仿真流程的兼容性，同时使搜索过程更加审慎、稳定和可解释。

Result: 在测试电路中，ACOF相比最强竞争基线，前10个品质因数平均提升了38.9%，遗憾值平均降低了24.7%。在个别电路上，品质因数峰值增益达到70.5%，遗憾值降低了42.2%。

Conclusion: 通过将迭代推理与仿真驱动搜索相结合，ACOF为在具有挑战性的设计空间中实现自动化模拟尺寸优化提供了一条更加透明的路径。该框架使搜索过程更加审慎、稳定和可解释，同时保持了与现有仿真流程的兼容性。
...
TL;DR: 该研究为"grokking"现象提供了可检验的有限尺寸相变证据，通过将群阶p作为外延变量，使用谱头尾对比作为序参量，应用凝聚态物理诊断方法证实了相变而非平滑交叉


<details>
  <summary>Details</summary>
Motivation: 现有研究常使用相变语言描述grokking现象（延迟泛化），但缺乏可证伪的有限尺寸输入。本研究旨在为grokking提供定量可检验的相变证据，而非仅作为类比

Method: 将群阶p作为可接受的外延变量，使用保留的谱头尾对比作为表示层序参量，应用凝聚态物理风格的诊断链：粗网格扫描和密集近临界加法审计，包括Binder类交叉分析

Result: Binder类交叉揭示了共享的有限尺寸边界，敏感性比较强烈反对平滑交叉解释（近临界审计中ΔAIC=16.8）。证实了grokking中相变语言可作为定量有限尺寸主张进行检验

Conclusion: grokking中的相变语言可作为定量有限尺寸主张进行检验，而不仅仅是类比，尽管目前相变阶数仍未解决。为机器学习中的相变研究提供了可证伪的框架
...
TL;DR: Worldline Slot Attention模型通过将对象建模为时空世界线中的持久轨迹，使用洛伦兹几何而非欧几里得几何来捕捉层次结构，在三个数据集上实现了6倍性能提升。


<details>
  <summary>Details</summary>
Motivation: 标准视觉模型将对象视为欧几里得空间中的独立点，无法捕捉部分与整体之间的层次结构。需要一种能够建模对象持久性和层次关系的架构。

Method: 引入Worldline Slot Attention，将对象建模为时空世界线中的持久轨迹。每个对象在不同层次级别有多个槽，共享相同空间位置但具有不同的时间坐标。比较了欧几里得和洛伦兹几何两种世界线表示。

Result: 欧几里得世界线准确率仅为0.078（低于随机机会0.33），而洛伦兹世界线在三个数据集上达到0.479-0.661准确率，实现了6倍改进，并在20多次独立运行中复现。洛伦兹几何也优于双曲嵌入。

Conclusion: 层次对象发现需要编码非对称因果关系的几何结构，这是欧几里得空间缺乏但洛伦兹光锥自然具备的归纳偏置。仅用11K参数即可实现这一效果。
...
TL;DR: Transformer架构理论上能够实现搜索算法，通过训练可以近似不同搜索策略，并在未见条件下泛化，任务导向的持续训练能释放预训练LLM的完整搜索能力。


<details>
  <summary>Details</summary>
Motivation: 虽然外部搜索算法能增强LLM的问题解决能力，但需要外部组件会使整体过程复杂化。研究旨在探索LLM或其底层Transformer架构是否能够近似实现搜索算法，从而简化问题解决流程。

Method: 引入"未知树搜索与老虎机反馈"简化框架，其中树扩展和反馈信号由外部指定。在此设置下，从零开始训练Transformer模型来近似不同的搜索策略，并通过任务导向的持续训练微调预训练LLM。

Result: Transformer在理论上具有足够的表达能力来实现不同的搜索策略，并能从零训练来近似这些策略。模型展现出对更长视野或更深树等未见条件的泛化能力。任务导向的持续训练能释放预训练LLM的完整搜索能力。

Conclusion: LLM的Transformer架构能够近似搜索算法，这为简化问题解决流程提供了可能，通过适当的训练方法可以释放其完整的搜索能力。
...
TL;DR: 本文提出了一个评估临床预测任务中可解释性方法的综合基准，分析了注意力机制、黑盒解释器等在不同模型架构和临床任务中的表现，并提供了改进临床预测管道可解释性的指导原则。


<details>
  <summary>Details</summary>
Motivation: 临床决策需要明确的可解释性来审计深度学习模型，但现有基准缺乏可扩展性和可重复性，未能系统研究可解释性在临床任务和模型架构间的变化。需要填补这一空白，评估不同可解释性方法在多样化临床预测任务中的表现。

Method: 构建了一个综合基准，评估多种可解释性方法在不同临床预测任务和模型架构上的表现。通过PyHealth开源框架提供可重复和可扩展的实现，系统分析了注意力机制、KernelSHAP、LIME等解释方法的性能。

Result: 研究发现：(1) 适当利用注意力机制是忠实解释模型预测的高效方法；(2) KernelSHAP和LIME等黑盒解释器在时间序列临床预测任务中计算不可行；(3) 多种可解释性方法可靠性不足，难以信任。基于这些发现提出了改进临床预测管道可解释性的指导原则。

Conclusion: 本研究填补了临床深度学习可解释性评估的空白，提供了系统化的基准框架。研究结果表明需要谨慎选择可解释性方法，并提出了改进临床预测管道可解释性的实用指南。通过PyHealth开源框架支持研究的可重复性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有黎曼流匹配方法需要在齐次空间上定义预度量和测地线，计算复杂且受限于特定几何结构，需要更简单、快速且完全内蕴的框架

Method: 通过将齐次空间上的数据分布提升到其底层李群上，将问题转化为李群上的流匹配任务，进而简化为李代数上的欧几里得流匹配，避免直接处理齐次空间的复杂几何

Result: 相比黎曼流匹配，该方法无需定义和计算预度量或测地线，实现了更简单、快速且完全内蕴的框架，能够处理更广泛的齐次空间

Conclusion: 提出的框架成功将流匹配扩展到齐次空间，通过李群提升策略简化了几何复杂性，为在复杂几何结构上进行生成建模提供了更高效的方法
...
TL;DR: 该论文提出了一种多答案强化学习方法，用于训练语言模型在推理时对多个答案进行分布推理，避免传统后处理将答案分布坍缩到单一主导模式的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界任务（如医疗诊断、模糊问答、信息不完整场景）通常涉及多个有效答案或不可约的不确定性，但现有语言模型后处理过程往往将答案分布坍缩到单一模式，无法生成多个合理假设并提供置信度估计。

Method: 修改强化学习目标，使模型能够在单次前向传递中显式生成多个候选答案，将推理时搜索的某些方面内化到模型的生成过程中，实现分布推理。

Result: 在问答、医疗诊断和编程基准测试中，相比单答案训练基线，该方法在多样性、覆盖率和集合级校准分数方面均有提升。模型生成多个答案所需token更少，在编程任务中准确性显著提高。

Conclusion: 多答案强化学习为推理时扩展程序（如best-of-k）提供了原则性且计算高效的替代方案，使语言模型能够更好地处理具有多个有效答案的现实世界任务。
...
<details>
  <summary>Details</summary>
Motivation: 仓库分拣系统需要实时人员配置决策优化，传统方法存在效率瓶颈，需要探索AI辅助决策的不同抽象层次方法。

Method: 1) 使用离线强化学习训练基于Transformer的策略，基于详细历史状态表示；2) 探索LLM在抽象、人类可读状态描述上的应用，系统比较提示技术、自动提示优化和微调策略。

Result: 1) Transformer策略在学习的模拟器中比历史基线提升2.4%吞吐量；2) LLM方法中，监督微调结合Direct Preference Optimization在手工模拟器中达到或略超历史基线性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决视频时序定位中的关键挑战：实现时间敏感的视文对齐，同时保持对不相关背景上下文的鲁棒性。现有方法在处理上下文干扰和假阴性样本方面存在不足。

Method: 1. 查询感知上下文多样化(QCD)：基于视频-文本相似性构建替换片段池的数据增强策略，仅混合语义不相关内容，避免查询无关混合导致的假阴性。
2. 上下文不变边界判别(CBD)损失：对比损失函数，在具有挑战性的时间边界处强制语义一致性，使其表示对上下文变化和困难负样本具有鲁棒性。
3. 上下文增强Transformer编码器(CTE)：分层架构，结合窗口化自注意力、双向交叉注意力和可学习查询，捕捉多尺度时间上下文。

Result: 在主要VTG基准测试(QVHighlights和Charades-STA)上实现最先进的性能。Recall@1(R1)分数相比现有最佳方法提升约5个点，显著减轻了假阴性问题。

Conclusion: CVA框架通过数据中心和架构增强的协同作用，有效解决了视频时序定位中的上下文干扰问题。查询感知上下文多样化、上下文不变边界判别损失和上下文增强Transformer编码器的组合显著提升了模型对不相关背景的鲁棒性和定位准确性。
...
TL;DR: 该论文解决了在线凸优化中两点反馈的强凸损失函数的高概率遗憾界问题，首次实现了与时间T和维度d都最优的O(d(log T + log(1/δ))/μ)遗憾界。


<details>
  <summary>Details</summary>
Motivation: 在线凸优化中，两点反馈下的强凸损失函数高概率遗憾界一直是一个未解决的开放问题。虽然两点反馈允许梯度估计，但带反馈梯度估计器的重尾特性使得标准集中分析变得困难，导致难以获得紧致的高概率遗憾界。

Method: 论文通过解决带反馈梯度估计器的重尾特性问题，开发了新的分析方法来获得高概率遗憾界。具体方法涉及对强凸损失函数的两点反馈机制进行优化，克服了标准集中分析的困难。

Result: 首次实现了μ-强凸损失函数的高概率遗憾界O(d(log T + log(1/δ))/μ)，该结果在时间范围T和维度d方面都是极小极大最优的。

Conclusion: 该研究解决了在线凸优化中两点反馈下强凸损失函数高概率遗憾界的开放问题，提供了最优的遗憾界，填补了该领域的重要理论空白。
...
Method: 提出"认知压缩"原则，强调通过架构设计而非后验权重惩罚来强制简约性。引入"制度指数"区分"转移制度"（不稳定、数据贫乏）和"稳定制度"（不变、数据丰富），指导模型复杂度选择。

Result: 在15个高风险领域的探索性综合研究中，制度指数与经验上优越的建模策略在86.7%的案例中（13/15）一致，验证了该框架的有效性。

Conclusion: 高风险AI需要从为扩展而扩展转向原则性简约。模型鲁棒性源于模型复杂度与数据"保质期"的匹配，而非参数数量增加。架构设计应使表示超出数据证据的方差在结构上代价高昂。
...
Method: 利用XTuner和LMDeploy基础设施支持，实现万亿参数级别的高效强化学习训练；采用多模态架构，整合通用与科学专业知识；通过严格精度一致性机制确保训练与推理对齐。

Result: 模型在通用能力上达到开源模型顶级水平，在化学、材料、生命科学、地球科学等关键科学领域的100多个专业任务上超越专有模型；实现了万亿参数规模的高效训练与部署。

Conclusion: Intern-S1-Pro作为"可专业化的通才"模型，成功融合了通用智能与专业科学智能，在万亿参数规模上实现了科学多模态基础模型的突破，为科学AI应用提供了强大工具。
...
TL;DR: SIGMA通过token级对比学习解决分子图线性化表示中的模态不匹配问题，结合IsoBeam搜索消除推理时的同构冗余，实现序列可扩展性与图保真度的统一。


<details>
  <summary>Details</summary>
Motivation: 线性化字符串表示是自回归分子生成的基础，但存在模态不匹配问题：单个分子图对应多个不同序列。这种歧义导致轨迹发散，即结构相同的部分图因线性化历史不同而潜在表示漂移。

Method: 提出SIGMA方法：不改变线性表示，而是通过token级对比目标使模型严格识别几何对称性，显式对齐共享相同后缀的前缀的潜在状态。同时引入IsoBeam搜索，在推理时动态剪枝等价路径以消除同构冗余。

Result: 在标准基准测试中，SIGMA在序列可扩展性和图保真度之间架起桥梁，相比强基线在多参数优化中展现出更优的样本效率和结构多样性。

Conclusion: SIGMA解决了线性化分子生成中的模态不匹配问题，通过对比学习和动态剪枝实现了高效且结构保真的分子生成，为可扩展的自回归分子设计提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 机器学习模型在水文应用中虽然能实现高预测精度，但通常缺乏物理可解释性。MCP提供了一个物理感知的AI框架，能够强制执行守恒原理，同时允许从数据中学习水文过程关系。
...
Result: 逐步增强MCP单元内部物理结构通常能提高预测性能。这些过程表示的影响具有强烈的水文气候依赖性：垂直排水显著提高干旱和雪主导流域的模型技能，但在降雨主导区域降低性能；地表积水影响相对较小。性能最佳的MCP配置接近LSTM基准的预测技能，同时保持明确的物理可解释性。
...
TL;DR: SEVerA框架通过形式化防护生成模型(FGGM)为自演化LLM智能体提供形式化安全保证，结合搜索-验证-学习三阶段确保零约束违反同时提升任务性能


<details>
  <summary>Details</summary>
Motivation: 现有自演化LLM智能体框架缺乏形式化安全性和正确性保证，在未知输入上自主执行时存在可靠性和安全隐患，需要将智能体代码生成转化为带约束的学习问题

Method: 提出形式化防护生成模型(FGGM)，允许规划器LLM为一阶逻辑形式化输出契约指定生成模型调用；基于FGGM构建SEVerA三阶段框架：搜索合成含FGGM调用的参数化程序，验证证明对所有参数值的硬约束正确性，学习应用梯度优化提升软目标

Result: 在Dafny程序验证、符号数学合成和策略合规智能体工具使用(τ²-bench)任务上，SEVerA实现零约束违反，性能优于无约束和SOTA基线，形式化行为约束不仅保证正确性还引导合成更高质量智能体

Conclusion: SEVerA框架成功将形式化保证与学习相结合，证明形式化行为约束既能确保智能体安全正确性，又能引导合成过程获得更高质量解决方案，为自演化LLM智能体提供可靠基础
...
TL;DR: 为鲁棒支持向量机（R-SVM）开发安全样本筛选规则，在不影响最优解的前提下降低训练复杂度，首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型。
...
Conclusion: 成功开发了针对鲁棒支持向量机的安全样本筛选规则，首次将安全筛选技术应用于监督机器学习中的最坏情况鲁棒模型，有效降低了计算复杂度而不影响模型性能。
...
TL;DR: 将离线强化学习中的Decision Transformer应用于旅行商问题，通过Pointer Network处理可变动作空间，使用期望分位数回归进行乐观条件化，在训练数据中的四种经典启发式算法基础上实现性能超越。


<details>
  <summary>Details</summary>
Motivation: 神经组合优化方法依赖在线强化学习，限制了实际部署，且未能充分利用数十年的算法知识积累。需要开发能够直接从启发式解数据集中学习并超越这些解的方法。

Method: 应用离线强化学习框架Decision Transformer，结合Pointer Network处理实例依赖的可变动作空间（节点选择），采用期望分位数回归对Return-to-Go进行乐观条件化，以处理最优值差异大的实例。

Result: 实验表明，该方法在旅行商问题上持续产生比训练数据中的四种经典启发式算法更高质量的路径，证明了离线强化学习能够解锁并超越现有领域知识中嵌入的性能。

Conclusion: 离线强化学习为组合优化问题提供了有前景的途径，能够从现有启发式解中学习并超越它们，克服了在线强化学习的部署限制，更好地利用了算法知识积累。
...
<details>
  <summary>Details</summary>
Motivation: 权重剪枝是压缩大语言模型的标准技术，但其对学习到的内部表征的影响仍不清楚。研究旨在系统理解剪枝如何重塑语言模型的特征几何结构。
...
Result: 最显著的发现是罕见SAE特征（低激活率）比高频特征更能抵抗剪枝，在17个实验条件中有11个显示Spearman相关系数ρ=-1.0。Wanda剪枝比幅度剪枝能更好地保留特征结构（最多3.7倍），预训练的SAEs在Wanda剪枝模型上到50%稀疏度仍有效。几何特征存活率不能预测因果重要性。

Conclusion: 剪枝起到隐式特征选择作用，优先破坏高频通用特征而保留专门的罕见特征。特征几何存活与因果重要性之间的分离对压缩下的可解释性具有重要意义。
...
TL;DR: 该研究提出基于范畴论的形式化框架来评估深度研究智能体，开发了包含296个问题的机制感知基准，揭示了当前AI在复杂结构信息处理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前深度研究智能体的评估主要依赖临时性的经验基准，这些启发式方法无法严格建模智能体行为，也不能充分压力测试长时程信息合成和模糊性解决能力。需要建立理论严谨的评估框架来系统性地测试智能体的结构理解能力。

Method: 使用范畴论形式化深度研究智能体行为，将深度研究工作流建模为结构保持映射（函子）的组合。基于此理论框架，开发了包含296个问题的机制感知基准，从四个可解释维度压力测试智能体：遍历顺序连接链、验证V结构拉回中的交集、对检索子结构施加拓扑排序、通过Yoneda探针进行本体证伪。

Result: 对11个领先模型的严格评估显示基线持续偏低，最先进模型平均准确率仅为19.9%，暴露了形式化结构压力测试的难度。研究发现当前AI能力存在明显二分：先进深度研究流水线能成功重新定义动态拓扑排序并展现稳健的本体验证能力，但在多跳结构合成方面几乎普遍崩溃。不同任务间的巨大性能差异表明智能体仍依赖脆弱的启发式方法而非系统理解。

Conclusion: 虽然顶级自主智能体现在能够有机统一搜索和推理，但实现复杂结构信息的泛化掌握仍然是一个严峻的开放挑战。该工作为深度研究智能体提供了理论严谨的评估框架，揭示了当前系统在结构理解方面的根本局限性。
...
TL;DR: Hi-MLIP是一种Hessian-informed机器学习原子间势，通过HINT高效训练协议显著减少Hessian标签需求，实现了势能面曲率的可靠捕获，显著提升了过渡态搜索和吉布斯自由能预测精度。


<details>
  <summary>Details</summary>
Motivation: 势能面的局部曲率对于从第一性原理预测分子和材料的实验可观测至关重要，但对于复杂系统仍然遥不可及。现有方法在获取Hessian矩阵（二阶导数）方面计算成本过高，限制了曲率相关现象的分析能力。

Method: 提出了Hessian-informed机器学习原子间势（Hi-MLIP），开发了高效训练协议HINT（Hessian INformed Training），包括Hessian预训练、构型采样、课程学习和随机投影Hessian损失等技术，将昂贵Hessian标签的需求降低了2-4个数量级。

Result: Hi-MLIP显著改善了过渡态搜索，使吉布斯自由能预测接近化学精度（尤其在数据稀缺情况下）。该框架还能准确处理强非谐氢化物，再现声子重整化和超导临界温度，与实验结果高度一致，同时绕过了非谐计算的计算瓶颈。

Conclusion: 该工作建立了一条增强机器学习原子间势曲率感知能力的实用途径，在广泛系统中架起了模拟与实验可观测之间的桥梁，为热力学和动力学现象的准确分析提供了新工具。
...
TL;DR: GlowQ：一种用于量化LLM的组共享低秩近似方法，通过缓存每个输入共享组的单个共享右因子，仅恢复提供最高精度收益的组或层，减少参数和内存开销，同时保持层特定校正的表达能力。
...
Method: 提出GlowQ方法，采用组共享低秩近似：1）为每个输入共享组缓存单个共享右因子；2）仅恢复提供最高精度收益的组或层；3）每个输入共享组计算一次高精度投影并在其模块中重复使用。还提出选择性变体GlowQ-S，仅在提供最大收益的地方应用缓存的共享模块。
...
Conclusion: GlowQ通过组共享低秩近似有效解决了量化LLM中精度损失与计算开销的权衡问题，在保持精度的同时显著降低了延迟和内存开销，为高效部署量化大语言模型提供了新方法。
...
TL;DR: FB-MEBE：一种用于四足机器人控制的在线零样本强化学习算法，结合无监督行为探索和正则化批评器，无需下游任务奖励即可学习多样化的策略，可直接部署到硬件。
...
Method: 提出FB-MEBE算法：1）基于Forward-Backward算法框架；2）引入无监督行为探索策略，通过最大化实现行为分布的熵来促进探索；3）添加正则化批评器，使恢复的策略更自然、更符合物理规律。

Result: FB-MEBE在模拟下游任务中优于其他探索策略，且生成的策略自然、可直接部署到硬件而无需微调。通过视频和代码验证了其有效性。

Conclusion: FB-MEBE通过结合行为探索和正则化，解决了零样本RL中数据多样性不足的问题，实现了可直接硬件部署的自然策略，为机器人控制提供了实用的在线零样本RL解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法在预测野火发生时，标准评估指标对火灾危险指数预报的操作性能衡量有限，且评估常未充分考虑误报率，而误报率在实际操作中至关重要

Method: 重新审视每日火灾危险指数模型评估范式，提出与真实世界决策对齐的森林火灾预报模型评估新方法，系统评估火灾活动预测准确性和误报率，并采用机器学习模型集成方法

Result: 机器学习模型集成能同时改善火灾识别和减少误报，证明了所提评估方法的有效性

Conclusion: 需要重新思考火灾预报模型的评估范式，采用与操作决策对齐的评估方法，机器学习模型集成是提高火灾危险指数预报性能的有效途径
...
TL;DR: Causal-INSIGHT是一个模型无关的后处理解释框架，用于从已训练的时序预测器中提取模型隐含的、有向的、有时滞的影响结构，通过干预式输入钳制分析预测器的响应。


<details>
  <summary>Details</summary>
Motivation: 理解多元时间序列中的有向时序交互对于解释复杂动态系统及其训练的预测模型至关重要。现有方法通常关注数据生成过程的因果结构推断，而非模型隐含的依赖关系。

Method: 提出Causal-INSIGHT框架：1）在推理时对预训练预测器应用系统性的、受干预启发的输入钳制；2）从这些响应中构建有向时序影响信号；3）引入Qbic准则，在无需真实图标签的情况下平衡预测保真度和结构复杂度。

Result: 在合成、模拟和真实基准测试中，Causal-INSIGHT能够泛化到不同的骨干架构，保持竞争力的结构准确性，并在应用于现有预测器时显著改善时序延迟定位性能。

Conclusion: Causal-INSIGHT提供了一种有效的方法来提取模型隐含的有向时序影响结构，为理解复杂动态系统中的预测模型依赖关系提供了实用的解释工具。
...
TL;DR: 该论文研究了音频到音频迁移学习，通过预训练不同模型状态并在三个下游任务上微调，发现预训练数据量和类别数对迁移学习有正面影响，但预训练与下游任务的相似性影响更大。


<details>
  <summary>Details</summary>
Motivation: 尽管迁移学习在深度学习中广泛应用且效果明显，但其内在工作机制、何时有效以及效果如何仍存在许多开放性问题。本研究旨在通过音频到音频迁移学习的系统性实验来深入理解这些机制。

Method: 在AudioSet的（基于本体的）子集上预训练各种模型状态，然后在三个计算机听觉任务上进行微调：声学场景识别、鸟类活动识别和语音命令识别。通过控制预训练数据的样本数量和类别数量，研究这些因素对迁移学习效果的影响。

Result: 增加预训练数据的样本数量和类别数量都对迁移学习有积极影响。然而，预训练任务与下游任务之间的相似性通常比数据量和类别数的影响更大，这种相似性能够使模型学习到可比较的特征。

Conclusion: 迁移学习的效果不仅取决于预训练数据的规模和多样性，更重要的是预训练任务与目标任务的相似性。相似性能够促进特征的可迁移性，这在音频领域的迁移学习中尤为关键。
...
TL;DR: 本研究评估了三种轻量级可解释模型（SARIMAX、Facebook Prophet、NeuralProphet）在北京PM2.5小时预测中的性能，发现Facebook Prophet在滚动重拟合下表现最佳，而在线残差校正使SARIMAX在冻结模型下达到最低误差。
...
Method: 采用泄漏感知预测工作流，结合时间序列数据划分、预处理、特征选择和外生驱动建模。评估SARIMAX、Facebook Prophet和NeuralProphet三种模型家族，在两种自适应机制下测试：每周滚动向前重拟合和冻结预测加在线残差校正。

Result: 在滚动重拟合机制下，Facebook Prophet表现最佳（MAE 37.61，RMSE 50.10），且计算时间显著少于NeuralProphet。在冻结模型机制下，在线残差校正提升了Facebook Prophet和SARIMAX的性能，校正后的SARIMAX达到最低总体误差（MAE 32.50，RMSE 46.85）。NeuralProphet在两个机制下均表现较差且不稳定。
...
TL;DR: 论文分析了on-policy蒸馏在长序列生成中的问题，提出了改进方法：使用教师top-K局部支持匹配（截断反向KL+top-p采样+特殊token掩码），在数学推理和智能体任务中表现更优。


<details>
  <summary>Details</summary>
Motivation: 传统基于采样token的on-policy蒸馏在长序列生成中存在脆弱性：将分布匹配简化为单token信号，当学生生成的序列偏离教师常见前缀时变得不可靠。需要解决采样token OPD的三个失效模式。

Method: 1. 理论分析：token级OPD相对于序列级反向KL存在偏差，但方差界限更紧；2. 提出教师top-K局部支持匹配：截断反向KL目标 + top-p rollout采样 + 特殊token掩码；3. 通过玩具实验验证梯度方差与学习稳定性的权衡。

Result: 在单任务数学推理和多任务智能体+数学训练中，提出的方法相比采样token OPD：1. 优化更稳定；2. 下游性能更好；3. 解决了三个失效模式：单token信号不平衡、学生生成前缀上的教师指导不可靠、tokenizer或特殊token不匹配导致的失真。

Conclusion: 教师top-K局部支持匹配（截断反向KL+top-p采样+特殊token掩码）是on-policy蒸馏的有效改进，解决了长序列生成中的稳定性问题，在数学推理和智能体任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界CPS操作中不安全事件罕见，导致极端类别不平衡，降低安全预测器性能。传统重平衡技术对时间序列CPS遥测数据效果不佳，而CPS行为不确定性（决策中的怀疑程度）与安全结果相关但尚未在安全监控中探索。

Method: U-Balance首先训练基于GatedMLP的不确定性预测器，将每个遥测窗口总结为分布运动学特征并输出不确定性分数。然后应用不确定性引导的标签重平衡机制，将具有异常高不确定性的"安全"标签窗口概率性地重新标记为"不安全"，从而在不合成新数据的情况下用信息丰富的边界样本丰富少数类。最后在重平衡数据集上训练安全预测器。
...
Conclusion: U-Balance通过利用行为不确定性有效解决了CPS安全监控中的极端类别不平衡问题，基于GatedMLP的不确定性预测器和uLNR机制都对其有效性有显著贡献，为CPS安全监控提供了新方法。
...
TL;DR: 将确定性数据访问视为随机采样的极限情况，建立统一框架分析AI系统中数据与随机性需求，提出内存级评估标准，分析传统架构局限并探索概率计算内存方案。


<details>
  <summary>Details</summary>
Motivation: 可信AI日益依赖概率计算实现鲁棒性、可解释性、安全性和隐私性。实际系统中，确定性数据访问与随机采样交织，性能瓶颈从算术单元转移到必须同时提供数据和随机性的内存系统。

Method: 提出统一的数据访问视角，将确定性访问视为随机采样的极限情况，使两种模式能在共同框架下分析。基于此定义内存级评估标准：统一操作、分布可编程性、效率、对硬件非理想性的鲁棒性以及并行兼容性。

Result: 分析显示增加随机性需求会降低有效数据访问效率，可能使系统进入熵受限操作。使用定义的标准分析了传统架构的局限性，并研究了新兴的概率计算内存方法。

Conclusion: 通过将采样与内存访问集成的概率计算内存方法，为可信AI的可扩展硬件提供了发展路径，解决了传统架构在同时满足数据和随机性需求方面的不足。
